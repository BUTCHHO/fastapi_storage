...
class UserGetter:
    def __init__(self, user_reader, session_reader,session_actor, cacher, time_handler):
    ...
    async def get_by_session_id(self, session_id):
        ...
        if user_id is not None:
            return await self.get_user_if_in_cache(int(user_id))
    ...
    async def _raise_and_delete_session_if_invalid(self, session):
        ...
        if not self.time_handler.is_date_future(session.expire_date):
            ...
            await self.session_actor.delete_by_kwargs(id=session.id)
